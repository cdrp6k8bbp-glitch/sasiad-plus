...
schema_json="$(wrangler d1 execute sasiad-plus-db-staging \
  ...
  --command "SELECT CASE WHEN
    ...
    AND EXISTS (SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'push_subscriptions')
    ...
    AND EXISTS (SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'data_retention_runs')
    AND EXISTS (SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'search_alerts')
    AND EXISTS (SELECT 1 FROM pragma_table_info('search_alerts') WHERE name = 'radius')
    AND EXISTS (SELECT 1 FROM pragma_table_info('search_alerts') WHERE name = 'kind')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'archived_at')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'availability_weekdays')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'availability_slots')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'availability_dates')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'availability_start_time')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'availability_end_time')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'freshness_confirmed_at')
    AND EXISTS (SELECT 1 FROM pragma_table_info('listings') WHERE name = 'freshness_reminded_at')
